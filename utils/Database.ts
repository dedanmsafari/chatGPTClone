import { Chat, Message, Role } from '@/utils/types';

import { type SQLiteDatabase } from 'expo-sqlite';
import * as FileSystem from 'expo-file-system';

export async function migrateDbIfNeeded(db: SQLiteDatabase) {
  //?logging DB for debugging
  // console.log("Where we store the files", FileSystem.documentDirectory);
...
export const getMessages = async (db: SQLiteDatabase, chatId: number) => {
  return (
    ...
    ...message,
    role: (message.role as unknown as string) === 'bot' ? Role.Bot : Role.User,
  }));
};
